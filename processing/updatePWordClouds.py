...
for index in range (0, size):
  ...
  post_num = coords.GetCoordinate(1) - skip
...
while (size > 1):
  ...
  if (mpirank >= half):
    dest = mpirank - half
    mpicomm.send (tfVectorPython, dest=dest, tag = size)
  else:
    source = mpirank + half
    remoteArray = mpicomm.recv (source=source, tag = size)
    for cluster in range (0, init_len):
      for term_index in range0:
        tfVectorPython[cluster][term_index] += remoteArray[cluster][term_index]
  size = half
...
if (mpirank == 0):
  for cluster in range (0, init_len):
    ...
    if (len(termsPython) != len(tfVectorPython[cluster])):
      print "There's a problem " + str(len(termsPython)) + " " + str(len(tfVectorPython[cluster]));
